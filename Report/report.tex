...
From this more general accuracy measure we make the observation that the model's guesses 
are often not far from the correct solution. This suggests that already small changes to the computations 
for analogy questions could have a positive impact on the accuracy. We propose two such small changes that 
can be easily applied to existing word2vec models (without retraining) and demonstrate their effectiveness on
a generalised question set.
...
model and techniques used for answering analogies. An outline of the experimental setup and the used test data is provided in Section \ref{sec:exp}. 
...
In \cite{levy2014linguistic} the same authors introduce an alternative interpretation of analogies and a corresponding similarity measure which is based on factors of cosine similarities. They demonstrate state-of-the-art results with their approach which they named $3CosMul$.



\begin{figure}[t]
\centering
\includegraphics[width=0.4\textwidth]{cbow}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.42\textwidth]{skip-gram}
...
\begin{equation}
	y = P_t(\boldsymbol{W'^T}h)
\end{equation}
with the softmax function $P_t$ of a $|V|$-dimensional vector $x$ given by 
\begin{equation}
	P_t(x)_i = \frac{e^{x_i}}{\sum_{j=1}^{|V|}{e^{x_j}}},
\end{equation}
for $i \in \{1, \hdots , |V|\}$.
The learnt word representations are then given by the rows of $\boldsymbol{W}$.
...
word in the vocabulary which has the maximal similarity in the embedding space. The 
...
\end{equation}
\begin{equation}
	L^1(x,y) = -{\sum_{i=1}^{n}{|(x_i-y_i)|}} 
\end{equation}
\begin{equation}
	Dice(x,y) = \frac{2x^T y}{||x||_2^2 + ||y||_2^2}
\end{equation}
\begin{equation}
	Cos(x,y) = \frac{x^T y}{||x||_2 \cdot ||y||_2}
\end{equation}
Note how some similarity measures are constructed by taking the negative of the distance between two vectors w.r.t. some distance function (\textit{e.g.} Euclidean distance in case of $L^2$).
We denote the approach in Equation \ref{eq:anal2} with cosine similarity by $cosAdd$ in the results.


\begin{figure*}[t]
\centering
\subfloat[Nearby points in the right are difficult to discriminate]{\includegraphics[width=0.25\linewidth]{meanshift1}
\label{a}}
\hfil
\subfloat[Shifting by the mean.]{\includegraphics[width=0.25\linewidth]{meanshift2}%
\label{b}}
\hfil
\subfloat[Reprojecting on the unit sphere.]{\includegraphics[width=0.25\linewidth]{meanshift3}%
\label{c}}
\hfil
\subfloat[Better discrimination of points]{\includegraphics[width=0.22\linewidth]{meanshift4}%
\label{d}}
...
The intuition behind this procedure is depicted in Figure \ref{fig:ms} which demonstrates how points that are very close get distributed out. Note that the word arithmetic to compute the query vector is performed after the normalisation (otherwise no change in the results would be expected). We denote this approach with $CosMS$ in our experiments.
...
\end{equation}
Note that these objectives all capture different aspects of the analogy, \textit{i.e.} they are not implying each other. We propose to combine these objectives in a weighted sum. Let $\phi$ denote the term in Equation \ref{eq:anal2}, \textit{i.e.}
\begin{equation}
...
\begin{equation}
	...
	\label{eq:alt}
\end{equation}
A value of $\lambda = 0.1$ for the weighting parameter has shown to perform well in our experiments (the model is quite robust to other choices for $\lambda$ though). We denote models using this similarity measure (including mean-shift) with $CosMS+$. 
...
The baseline model for the comparison is a Skip-Gram model with a learning rate $\alpha=0.025$, window size of 5 and vector size equal to 100. 
...
We observe that if we choose the learning rate too high (\textit{e.g.} above $2^{-3.5}$) the accuracy drops dramatically. The reason for this can be found in the gradient descent update rule, where a high value of $\alpha$ will make the gradients overshoot the minimum.
However, learning rates between ${2^{-5.5}}$ and ${2^{-4}}$ yielded good results for all models in our experiments.
...
$3CosMul$   &        43.4\%    &    20.5\%     &    20.6\%   &     56.5\%   &    13.2\%    &   11.8\%     &   39.7\%    &     73.5\%   &    28.6\%   &   39.9\%   &   39.5\%     \\
...
$CosMS+$   &        \textbf{49.2\%}    &    21.3\%     &    \textbf{24.1\%}   &     \textbf{65.4\%}   &    \textbf{15.5\%}    &   \textbf{16.0\%}     &   \textbf{43.9\%}    &     78.4\%   &    \textbf{33.9\%}    &   47.7\%    &   \textbf{44.0\%}     \\
 \hline
\end{tabular}
\caption{Comparison of different similarity-measures. Results obtained on the generalised question set of \cite{mikolov2013efficient} with a Skip-Gram model trained on the \textit{Text8} corpus. We can observe that cosine-similarity overall performs better compared to the other measures.  Although not a similarity-measure by its own we list the results of our proposed mean-shift ($CosMS$) and "combination of interpretations" ($CosMS+$) approaches for comparison. We observe slight increases in accuracy for both methods. Note that the total is computed as the micro-average (mean over all questions).} 
\label{tab:dists}
\end{table*}

\subsection{Similarity Measures}
To compare the different similarity measures introduced in Subsections \ref{sec:sim} and  \ref{sec:ms} we show results on selected categories of the generalised analogy dataset \cite{mikolov2013efficient} obtained with the baseline Skip-Gram model with parameters described in Section \ref{sec:baseline}.
...
In our comparison we also included results for the similarity measure $3CosMul$ as proposed in \cite{levy2014linguistic}. Contrary to their results, we do not observe an increase in accuracy using this approach. Most likely this is due to differing underlying corpora or other parameter choices. 
...
We evaluated how well word2vec models built from a large-scope corpus could answer domain specific questions.
For this purpose we used the baseline model described in Section \ref{sec:baseline}, using either the \textit{wiki} or \textit{text8} corpus.
We then measured the mean-shift accuracy of these models against the Brand-Origin and Movie-Director questions sets.

We also tried updating the existing models by training them using corpora closer related to the questions domains. We re-trained the models using the \textit{treebank} and \textit{movie\_reviews} corpora to answer Brand-Origin and Movie-Director questions, respectively.

Table \ref{tab:accu_treebank} and \ref{tab:accu_movie_reviews} show that the baseline large-scope model performs much worse on our domain-specific questions than on the generalised analogy dataset \cite{mikolov2013efficient},
in particular for movie related questions.
An explanation for this result is that many movie titles are made of common words,
cited in numerous different context. On the other hand,
a movie's title and director may be mentioned too far from each other in the corpus to fit in the window size.

Note that for both questions sets, the accuracy increases significantly with a larger model.
This improvement is more important for specialized questions than for general ones.


Updating existing models with more specific corpora worsened their accuracy in most cases. Applying this training multiple times amplified the effect.
It also harmed the models accuracy against the generalised analogy dataset.
However the \textit{treebank} and \textit{movie\_reviews} corpora are smaller than the original ones by orders of magnitude.
Therefore we cannot generalise the counter-productivity of such a re-training based on our results.
...
%TODO: Talk about combination of parameters variation?
All of our experiments were based on relatively small word2vec models
compared to the size of models often used in practice.
We can observe the importance and effects of the model size in our experiments, 
but we can not be sure how our results translate to much larger models. 
We tested with corpora of different kinds throughout this work,
but the larger corpora (\textit{text8} and \textit{wiki}) are both based on the same material (Wikipedia articles).
We can therefore not be sure how our results translate to corpora of similar size but different nature for example. 
Related is the question of how the results translate to corpora in another language.

Moreover, experimenting with two novel sets of questions seems insufficient to conclude on the model's performance on specific domains.
We also observe how counter-productive re-training can be, but this could be anecdotal.
This would become clearer if we updated the original model with a re-training corpus of similar size as the original corpus (requiring a large corpus).
We could then compare the accuracies of both models and decide whether one performs significantly worse than the other.

Lastly, we could not find a convincing theoretical explanation for the performance difference between similarity measures. Understanding the cause of such a difference could possibly  help finding better measures. 


\subsection{Future Work}
As explained in the previous section,
it would be interesting to train and evaluate models on much larger corpora to confirm the tendencies we exposed.
In particular, we need to check if improvements from mean-shift and the combination of interpretations translate to bigger models.

Another interesting direction of work is to figure out ways to explicitly train models with the goal of querying for analogies and similarities in mind. 

Finally we want to compare the performance of word2vec against other tools such as GloVe\footnote{Global Vectors for Word Representation} \cite{pennington2014glove} and models based on recurrent neural networks.
