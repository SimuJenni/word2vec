...
where the objective is to decide how similar two target word-pairs such as
...
and Figure~\ref{fig:skip}. Both architectures take one-hot encoded words as inputs
which get then mapped into into an $N$-dimensional vector via the weight matrix
$W$. The hidden unit then accumulates the sum of these vectors for all input
words. The output of the model is then generated multiplying the hidden-units by
...
likelihood of their output. Training is done via gradient descent using back-propagation
